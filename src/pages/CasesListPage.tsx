...
export default function CasesListPage() {
    const { firmCases, loadFirmCases, updateCase, error, loading } = useCases();

    const { user } = useAuth();

    const safeFirmCases: CaseRecord[] = firmCases ?? [];
    ...
    useEffect(() => {
        let result = safeFirmCases;

        if (activeFilter !== "All Cases") {
            const normalizedFilter = activeFilter.toLowerCase();
            result = result.filter(
                (firmCase) => firmCase.type?.toLowerCase() === normalizedFilter
            );
        }

        const query = searchQuery.trim().toLowerCase();
        if (query) {
            result = result.filter((firmCase) =>
                firmCase.caseNumber.toLowerCase().includes(query)
            );
        }
    ...
    }, [safeFirmCases, activeFilter, searchQuery]);
    ...
    function toggleStar(caseId: string, currentValue: boolean) {
        updateCase(caseId, { isStarred: !currentValue });
    }
    ...
    return (
        <PageWrapper>
            <HeroSection>
                <HeroContent>
                    <HeroTitle>
                        <TitleLine1>Your</TitleLine1>
                        <TitleLine2>Case Library</TitleLine2>
                    </HeroTitle>
                    <HeroDescription>
                        Manage and track all your legal cases effortlessly.
                    </HeroDescription>
                    <SearchBarWrapper>
            ...
            <ResultsSection>
                <BrowseHeader>
                    <BrowseTitle>
                        <BrowseHeading>Browse Your Cases</BrowseHeading>
                        <BrowseSubtext>
                ...
                {!loading && !error && (
                    <>
                        <CasesList>
                            {visibleCases.map((singleFirmCase) => (
                                <CaseCard
                                    key={singleFirmCase.id}
                                    caseData={singleFirmCase}
